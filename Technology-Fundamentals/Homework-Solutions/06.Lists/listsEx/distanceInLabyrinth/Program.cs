    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (queue.Count != 0)
            {
                ...
                if (row - 1 >= 0 && lab[row - 1, col] != "x" &&
                    visited[row - 1, col] == false)
                {
                    queue.Enqueue(new Cell(row - 1, col, false, current.Moves + 1));
                }

                if (col + 1 < lab.GetLength(1) && lab[row, col + 1] != "x" && visited[row,
                    col + 1] == false)
                {
                    queue.Enqueue(new Cell(row, col + 1, false, current.Moves + 1));
                }

                if (row + 1 < lab.GetLength(0) && lab[row + 1, col] != "x" &&
                    visited[row + 1, col] == false)
                {
                    queue.Enqueue(new Cell(row + 1, col, false, current.Moves + 1));
                }

                if (col - 1 >= 0 && lab[row, col - 1] != "x" && visited[row, col -
                    1] == false)
                {
                    queue.Enqueue(new Cell(row, col - 1, false, current.Moves + 1));
                }
